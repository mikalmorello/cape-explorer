...
### Requirement: Towns are outlined, colored by Cape region, and labeled
...
(Upper Cape, Mid Cape, Lower Cape, Outer Cape), and labeled with its
name. The ocean SHALL NOT be rendered from map data.
...
#### Scenario: Town name labels
- **WHEN** the map renders
- **THEN** each town's name appears as a text label positioned within
  its polygon

### Requirement: Inland water is deferred for now
Ponds, lakes, and rivers SHALL NOT render on the map for now; the data
pipeline that produces geometrically land-clipped inland water
(`src/data/capeWater.json`, generated by the `generate-water`
workflow) SHALL remain in the repo so the layer can be re-added later
without rebuilding the pipeline.

#### Scenario: No inland water rendered
- **WHEN** the map renders
- **THEN** no ponds, lakes, or rivers appear above the town fills,
  while `src/data/capeWater.json` and its generator remain present
...
### Requirement: Pin click shows location details in a side panel
Clicking a marker SHALL open a fixed-position panel overlaying the map
(not a popup anchored to the pin) showing the location's name, its
activities (title and category), and - when present - its area,
address, website link, notes, photo album link, and closed badge. The
panel SHALL have a close control, and clicking elsewhere on the map
SHALL also dismiss it. Selecting a different marker while the panel is
open SHALL replace its contents with the newly selected location.
...
- **THEN** the side panel opens showing the location name, each
  activity, and a clickable website link

#### Scenario: Location with no activities
- **WHEN** the user clicks the marker of a location with no activities
- **THEN** the side panel shows the location name (and area/notes if
  present) without an activities section

#### Scenario: Selecting another location while the panel is open
- **WHEN** a location's panel is open and the user clicks a different
  marker
- **THEN** the panel updates in place to show the newly selected
  location, without needing to close and reopen it
...
no transit markers, no road network, and no base-map text labels.
Only the app's own land rendering, its own town name labels, and
location pins appear.

#### Scenario: Map renders without competing content
- **WHEN** the map loads
- **THEN** no POI icons, transit markers, roads, or third-party
  base-map labels are visible - only the region-colored towns, town
  name labels, waves, and the app's pins
